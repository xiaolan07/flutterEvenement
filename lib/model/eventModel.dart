...
class EventModel {
  ...
  factory EventModel.fromJson(Map<dynamic, dynamic> json) {
    return EventModel(
      ...
      descriptionFr: json['description_fr'] as String? ?? 'Description non trouvée',
      imageUrl: json['image'] as String? ?? 'Image non trouvée',
      thematiques: List<String>.from(json['thematiques'] ?? ['Thématique non trouvée']),
     ...
     geolocalisation: (json['geolocalisation'] as Map<dynamic, dynamic>? ?? {}).entries.map((entry) => double.tryParse(entry.value.toString()) ?? 0.0).toList(),
      ...
      telephone:  json["telephone_du_lieu"] as String? ?? ' Pas de numero de téléphone 😢',
    );
  }
}
