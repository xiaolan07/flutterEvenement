...
class ParcoursDetailsPage extends StatefulWidget {
  ...
  const ParcoursDetailsPage({Key? key, required this.parcours})
      : super(key: key);
...
class _ParcoursDetailsPageState extends State<ParcoursDetailsPage> {
  ...
  bool _isNbLikeModified = false;

  @override
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        Navigator.pop(context, _isNbLikeModified);
        //// tradnsférer _isNbLikeModified lors de retourner sur la page parcours, il va MAJ le nbLike
        return true;
      },
      child: Scaffold(
        appBar: AppBar(
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: widget.parcours.titreEvents
                      .map((titreEvent) => Chip(
                ...
                Center(
                  child: ElevatedButton.icon(
                    onPressed: () {
                      setState(() {
                        _isLiked =
                            !_isLiked; // Change l'état de _isLiked à chaque clic
                      });
                      // Ajoutez ici votre logique pour le bouton J'aime
                      _firestoreService
                          .incrementNbJaime(widget.parcours.id)
                          .then((_) {
                        ...
                        _isNbLikeModified = true;
                        print("Nombre de 'J'aime' incrémenté avec succès");
                      }).catchError((error) {
                        print(
                            "Erreur lors de l'incrémentation du nombre de 'J'aime': $error");
                      });
                    },
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      backgroundColor: _isLiked
                          ? Colors.blue
                          : Colors.white, // Assure que le texte reste lisible
                    ),
                  ),
                ),
              ],
